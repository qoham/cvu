...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--unlearn_condition_dropout",
        type=float,
        default=0.1,
        help="unlearn conditioning dropout.",
    )
    parser.add_argument(
        "--retain_condition_dropout",
        type=float,
        default=0.1,
        help="retain conditioning dropout.",
    )
    parser.add_argument(
        "--alpha",
        type=float,
...
class MyDataset(Dataset):
    ...
    def __init__(
        ...
        size=512,
        center_crop=False,
        unlearn_condition_dropout=0.1,
        retain_condition_dropout=0.1,
    ):
        self.size = size
        ...
        self.unlearn_condition_dropout = unlearn_condition_dropout
        self.retain_condition_dropout = retain_condition_dropout
    ...
    def __getitem__(self, index):
        ...
        if example['unlearn']:
            target_prompt = ""
            target_image = Image.new("RGB", (self.size, self.size), color=(127, 127, 127))
            # target_prompt = "a image of dog"
            # target_image = Image.open("./dog.png").resize((self.size, self.size))
            if random.random() < self.unlearn_condition_dropout:
                source_prompt = ""
                target_prompt = ""
        else:
            if random.random() < self.retain_condition_dropout:
                source_prompt = ""
                target_prompt = ""
...
def train_main(args):
    ...
    train_dataset = MyDataset(
        ...
        size=512,
        center_crop=False,
        unlearn_condition_dropout=args.unlearn_condition_dropout,
        retain_condition_dropout=args.retain_condition_dropout,
    )
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("cvu", config=tracker_config)
    ...
    def log_image_validation(should_generate: bool):
        if accelerator.is_main_process:
            if args.validation_prompts is not None and should_generate:
                # create pipeline
    ...
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            log_image_validation(global_step == 0)
            ...
            with autocast("cuda"):
                ...
                noise_pred, intermediate_outputs = get_intermediate_outputs(
                    model=unet,
                    inputs={"sample": deepcopy(noisy_source_input), "timestep": timesteps, "encoder_hidden_states": deepcopy(encoder_hidden_states), "return_dict": False},
                ...
                with torch.no_grad():
                    ...
                    origin_target_noise_pred, origin_target_intermediate_outputs = get_intermediate_outputs(
                        model=origin_unet,
                        inputs={"sample": noisy_target_input, "timestep": timesteps, "encoder_hidden_states": target_encoder_hidden_states, "return_dict": False},
                        target_modules=target_modules
                    )
                    # cat 图片+cat prompt
                ...
                # 不同时间步使用不同权重
                timestep_weight = get_timestep_weight(timesteps, [(1 - a).item() for a in noise_scheduler.alphas_cumprod], max_timestep=max_timestep-1)

                # 中间层损失
                unlearn_intermediate_loss = 0.0
                retain_intermediate_loss = 0.0

                layer_weights = [0.16, 0.16, 0.17, 0.21, 0.36, 0.39, 0.46, 0.59, 0.96, 1.0]
                for level, (output, origin_output, origin_unlearn_output) in enumerate(zip(intermediate_outputs, origin_target_intermediate_outputs, origin_unlearn_source_intermediate_outputs)):
                    if unlearn.sum():
                        ...
                        assert len(origin_unlearn_output) == len(origin_output[unlearn])
                        target_point = origin_unlearn_output + (origin_output[unlearn] - origin_unlearn_output) * args.threshold_scale
                        threshold = F.mse_loss(origin_unlearn_output, target_point, reduction="none")
                        # threshold = threshold.mean(dim=tuple(range(1, len(threshold.shape))))
                        travel_distance = F.mse_loss(output[unlearn], origin_unlearn_output, reduction="none")
                        # travel_distance = travel_distance.mean(dim=tuple(range(1, len(travel_distance.shape))))
                        boundary_distance = torch.clamp(threshold - travel_distance, min=0.0)
                        boundary_distance = boundary_distance.mean(dim=tuple(range(1, len(boundary_distance.shape))))
                        weighted_boundary_distance = layer_weight * timestep_weight[unlearn] * boundary_distance
                        unlearn_intermediate_loss += weighted_boundary_distance.mean()
                    if (~unlearn).sum():
                ...
                intermediate_loss = args.alpha * unlearn_intermediate_loss + (1 - args.alpha) * retain_intermediate_loss

                # ground truth loss
                retain_gt_loss = 0.0
                if (~unlearn).sum():
                    retain_gt_loss = F.mse_loss(noise_pred[0][~unlearn].detach(), origin_target_noise_pred[0][~unlearn].detach())
                # print(f"--> retain_gt_loss: {retain_gt_loss}")
            ...
            accelerator.backward(intermediate_loss)
            ...
            log_image_validation(global_step % args.validation_steps == 0)
